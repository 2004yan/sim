...
Paddy robot drive model:
    ...
    - Rear wheel:     free spinning (velocity target 0)
...
REAR_WHEEL_SPEED = 0.0   # rad/s (free wheel - leave at 0)
...
class PaddyRobotController:
    ...
    def set_rear_wheel_speed(self, rad_s):
        if self.rear_wheel_idx is None:
            return
        self.controller.apply_action(ArticulationAction(
            joint_velocities=np.array([rad_s], dtype=np.float32),
            joint_indices=np.array([self.rear_wheel_idx], dtype=np.int32),
        ))
...
if __name__ == "__main__":
    ...
    bot.set_rear_wheel_speed(REAR_WHEEL_SPEED)
    print(f"[ctrl] commands sent: L={LEFT_SPEED}, R={RIGHT_SPEED}, "
          f"steer={np.degrees(REAR_LINK_ANGLE):.1f} deg, rear={REAR_WHEEL_SPEED}")
